import { addManualAsset, addManualCash, addManualLiability, createSnapshotAction } from './actions';
...
export default async function Home() {
  const [aggregation, snapshots, accounts, manualAssets, manualLiabilities, audits] = await Promise.all([
  ...
  return <main className="container">
    ...
    <section className="grid" aria-label="Main dashboard cards">
      ...
      <div className="card"><h3>Account breakdown</h3>{accounts.length ? accounts.map(a => <p key={a.id}>{a.name} · {a.provider.name}</p>) : <p className="muted">Manual account entries appear here.</p>}</div>
    </section>
    <h2>Manual entry</h2>
    <section className="forms">
      <div className="card"><h3>Add manual asset</h3><form action={addManualAsset}><input name="name" placeholder="Name" required/><input name="accountName" placeholder="Account"/><select name="assetClass" defaultValue="FUND"><option>CASH</option><option>CRYPTO</option><option>EQUITY</option><option>FUND</option><option>BOND</option><option>REAL_ESTATE</option><option>COMMODITY</option><option>OTHER</option></select><input name="currency" placeholder="Currency" defaultValue="EUR" required/><input name="quantity" type="number" step="any" placeholder="Quantity" required/><input name="unitPrice" type="number" step="any" placeholder="Unit price" required/><button>Add manual asset</button></form></div>
      <div className="card"><h3>Add manual liability</h3><form action={addManualLiability}><input name="name" placeholder="Name" required/><input name="accountName" placeholder="Account"/><input name="currency" placeholder="Currency" defaultValue="EUR" required/><input name="amount" type="number" step="any" placeholder="Amount" required/><button>Add manual liability</button></form></div>
      <div className="card"><h3>Add manual cash/bank balance</h3><form action={addManualCash}><input name="name" placeholder="Cash / bank balance name" required/><input name="accountName" placeholder="Account"/><input name="currency" placeholder="Currency" defaultValue="EUR" required/><input name="unitPrice" type="number" step="any" placeholder="Balance" required/><button>Add manual cash balance</button></form></div>
      <div className="card warning"><h3>Add Ethereum public address</h3><form action="/api/crypto-address" method="post"><input name="address" placeholder="0x public address only" required/><button>Add Ethereum public address</button></form><p className="muted">Public addresses only. The app never asks for crypto private keys or seed phrases. ERC-20 discovery can be extended with an Etherscan key later.</p></div>
    </section>
